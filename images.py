...
def normalize_skin_color(skin_color,gender):
...
def get_images(skin_color,gender):
    if "skin_data" not in st.session_state:
        st.session_state["skin_data"] = load_skin_data()

    if "selected_image_path" not in st.session_state:
        # Normalize the skin color
        normalized_skin_color,normalized_gender = normalize_skin_color(skin_color,gender)
        #print('Skin Data session:' ,st.session_state["skin_data"])
        print('Normalized Skin Color: ',normalized_skin_color,'Gender: ',normalized_gender)
        #print('Skin Data items: ', st.session_state["skin_data"].itemxs())

        # Filter keys based on normalized skin color
        matching_keys = [key for key, attributes in st.session_state["skin_data"].items() if attributes.get("skin") == normalized_skin_color and attributes.get("gender") == normalized_gender]
